...
import { 
  ...
  ChevronLeft, Check, MapPin, CheckCircle, XCircle 
} from 'lucide-react';
...
export default function ContactSection() {
  ...
  return (
    <section id="contact" className="relative py-24 md:py-32 bg-black overflow-hidden px-6">
        ...
        <div className="grid lg:grid-cols-2 gap-20 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
            ...
            <SectionTitle className="text-left mb-8">
              DÉMARREZ UN <br />
              <span className="text-gradient italic font-script lowercase text-6xl">Projet avec nous.</span>
            </SectionTitle>
          ...
          {/* Form Section */}
          <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="bg-white/[0.02] border border-white/10 rounded-[3rem] p-8 md:p-10 backdrop-blur-xl shadow-2xl relative overflow-hidden">
              ...
              {steps.map((step, i) => (
                <div key={i} className="flex flex-col items-center gap-2">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 transition-all duration-500 ${
                    i <= currentStep ? (currentStep === 2 ? 'border-green-500 bg-green-500' : 'border-accent bg-accent') : 'border-white/10 bg-black'
                  }`}>
              ...
              <AnimatePresence mode="wait">
                ...
                {currentStep === 2 && (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} key="step2">
                    <textarea value={formData.message} onChange={(e) => setFormData({...formData, message: e.target.value})} placeholder="Comment pouvons-nous vous aider ?" className="w-full h-40 bg-white/5 border border-white/10 rounded-2xl p-5 text-sm focus:border-green-500 outline-none transition-all placeholder:text-white/10 resize-none font-sans" />
                  </motion.div>
                )}
              </AnimatePresence>
      ...
      <AnimatePresence>
        {showPopup.show && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-black border border-white/10 p-10 rounded-[3rem] max-w-sm w-full text-center shadow-2xl backdrop-blur-2xl">
               ...
               <h3 className="text-2xl font-black uppercase tracking-tighter mb-2">{showPopup.success ? 'Message Envoyé' : 'Erreur'}</h3>
               <p className="text-gray-400 text-sm mb-8 leading-relaxed">
                 {showPopup.success ? 'Merci! Notre équipe reviendra vers vous très vite.' : 'Veuillez vérifier les champs Cc/Bcc f EmailJS.'}
               </p>
               <button onClick={() => setShowPopup({ ...showPopup, show: false })} className="w-full py-4 bg-white text-black font-black uppercase text-[10px] tracking-widest rounded-2xl">Fermer</button>
            </motion.div>
          </div>
        )}
      </AnimatePresence>
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="relative">
        ...
        <input type="text" value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} className="w-full bg-white/5 border border-white/10 rounded-xl py-3 pl-12 pr-5 text-sm focus:border-accent outline-none transition-all placeholder:text-white/5 font-sans" />
      </div>
    </div>
  );
}